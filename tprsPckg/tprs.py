...
def get_complete_tprs_data():
   ...
   t.entryno,
   t.date,
   pd.description AS detaildescrip,
   brand.name AS brandname,
   inv.unitname  AS invoiceunitname,
   sub_cat.name AS subcatname
FROM
    tblitems i
LEFT JOIN
    tbltrp_items pd ON i.itemid = pd.finish_itemid
LEFT JOIN
tblproduction_detail p ON pd.trp_itemid = p.itemid
LEFT JOIN
tblproduction p ON p.entryno = p.entryno
LEFT JOIN
tbl_section s ON t.deptid = s.department_id
LEFT JOIN
tblitemcategory cat ON i.categoryid = cat.id
LEFT JOIN
pro_sub_cat sub_cat ON i.subcatid = sub_cat.id
LEFT JOIN
tblinventoryunitname inv ON i.masterpackingunitid = inv.id
LEFT JOIN
pro_item_brand brand ON i.brandid = brand.id
WHERE t.date >= '2021-01-01' AND t.deptid = 36;
...
def render_tprs(username):
    ...
    selection = option_menu(
                menu_title = "TPRS Department",
                options = ['TPRS Operations', 'Last Working Day'],
                icons = ["body-text", "alarm-fill"],
                menu_icon = ["palette"],
                orientation = "horizontal"
    )
    try:
        if selection == "TPRS Operations":
            render_tprs_operations(username)
