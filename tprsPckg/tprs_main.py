...
from tprsPckg.tprs_helper import preprocess_tprs, preprocess_tprs_lastworking

def render_tprs_operations(username):
    st.markdown(hover_size(), unsafe_allow_html=True)
    st.write(fh.format_st_button(), unsafe_allow_html=True)
    with st.spinner("TPRS Dashboard is Getting Ready for you... 🖨️"):
        complete_df = tprs.get_complete_tprs_data()
    ...
    complete_df_t, total_output, total_target, total_co, total_percent_co, avg_efficiency, avg_throughput , sorted_months, co_exe_by_month,eff_by_month, throughput_by_month, customer_wise_eff_thr, customer_qty, co_by_brand, station_wise_eff_thr, co_by_station = preprocess_tprs(
...
def render_recent_working_day(username):
    st.markdown(hover_size(), unsafe_allow_html=True)
    complete_df_for_lwd = tprs.get_complete_tprs_data()
    ...
    complete_df_t, total_output, total_target, total_co, total_percent_co, avg_efficiency, avg_throughput , sorted_months, executions_by_month ,eff_by_month, throughput_by_month, customer_wise_eff_thr, mastercode_qty, co_by_brand, planned_vs_target, station_wise_eff_thr, co_by_station = preprocess_tprs_lastworking(
        complete_df)
