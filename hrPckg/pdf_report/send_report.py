...
LOGS_DIR = "logs"
FILE = "hr_pdf_logs.log"
class SendReport:
    def __init__(self):
        self.logger = AppLogger()
        self.LOGS_BASE_DIR = Path(os.path.join("hrPckg",'pdf_report', LOGS_DIR)).resolve().absolute()
        ...
        self.pdf_files = [f"HR Report on {self.date_vn}.pdf"]

    def send_email(self):

        try:
            GenerateReport(f"HR Report on {self.date_vn}.pdf", self.logs_file).complete_report()
            LoadData(load_data=False).remove_csvs()
            msg = EmailMessage()
            msg["Subject"] = "HR Daily Status Report"
            ...
            LoadData(load_data=False).remove_csvs()
            alert = Email()
            alert.pipeline_failure_alert(subject="HR PDF Report Pipeline Failed", error = str(e))
            with open(self.logs_file, 'a+') as file:
