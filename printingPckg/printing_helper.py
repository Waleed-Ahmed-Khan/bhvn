...
def preprocess_printing(complete_df, start_date, end_date, year, po_selection, customer_selection, station_selection, mastercode_selection, size_selection):
    mask =  (complete_df['date'].astype('datetime64').dt.date.between(start_date, end_date)) & (complete_df['year'] == year) & (complete_df['customer_PO'].isin(po_selection)) & (complete_df['SubCategory'].isin(customer_selection)) & (complete_df['station'].isin(station_selection)) & (complete_df['mastercode'].isin(mastercode_selection)) & (complete_df['size'].isin(size_selection))
    ...
    station_wise_throughput = complete_df.groupby('station', as_index=False)['Hourly Throughput'].mean()
    station_wise_eff = complete_df.groupby('station', as_index=False)['% Eff'].mean()
    station_wise_eff_thr = pd.merge(left=station_wise_throughput, right=station_wise_eff, how='left').sort_values(by='% Eff', ascending = False)
    ...
    co_by_station = complete_df.groupby(by='station', as_index=False)['C/O'].sum()
    
    return complete_df, total_output, total_target, total_co, total_percent_co, avg_efficiency, avg_throughput , sorted_months, co_exe_by_month,eff_by_month, throughput_by_month, customer_wise_eff_thr, customer_qty, co_by_brand, station_wise_eff_thr, co_by_station

@st.cache(suppress_st_warning=True, allow_output_mutation=True, ttl = 500)
def preprocess_printing_lastworking(complete_df):
    total_output = complete_df['qty'].sum()
    total_target = complete_df['target'].sum()
    total_co = complete_df['C/O'].sum()
    ...
    mastercode_qty = complete_df.groupby(by='mastercode', as_index=False)['qty'].sum().rename(columns={'qty': 'Quantity'})
    co_by_brand = complete_df.groupby(by='SubCategory', as_index=False)['C/O'].sum()
    co_by_station = complete_df.groupby(by='station', as_index=False)['C/O'].sum()
    co_by_station = co_by_station[co_by_station['C/O'] != 0]

    planned_vs_target = complete_df.groupby('station', as_index=False)[['qty','target']].sum().rename(columns={'qty':'Output', 'target':'Target'}).astype({'Output':int, 'Target': int})
    
    return complete_df, total_output, total_target, total_co, total_percent_co, avg_efficiency, avg_throughput , sorted_months, executions_by_month ,eff_by_month, throughput_by_month, customer_wise_eff_thr, mastercode_qty, co_by_brand, planned_vs_target, station_wise_eff_thr, co_by_station