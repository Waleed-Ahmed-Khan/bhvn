...
def barchart(df, values, categories, title = 'Bar Chart', orientation='h', texposition = "auto", text_color ='white'  ):
    if orientation == 'h':
        data_labels = values
    else:
...
def pie_chart(df, values, categories, title='Pie Chart', hover_name = None, hover_data = None):
    fig = px.pie(data_frame=df, values=values,names=categories,
        ...
        hover_name=hover_name,              #values appear in bold in the hover tooltip
        hover_data=hover_data,            #values appear as extra data in the hover tooltip
